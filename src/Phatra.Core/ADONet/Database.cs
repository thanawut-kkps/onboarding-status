...
namespace Phatra.Core.AdoNet
{
    ...
    /// <para>usage: </para>
    /// <para>using (var db = new Database()) {};                                 </para>
    /// <para>using (var db = new Database(connectionString)) {};                 </para>
    /// <para>using (var db = new Database(connectionString, providerName)) {};   </para>
    /// <para>using (var db = Database.FromConfig());                             </para>
    /// <para>using (var db = Database.FromConfig(connectionStringName));         </para>
    /// <para>
    ...
    public class Database : IDatabase
    {
        ...
        public Database(IDbConnection connection, DbConfig config)
        {
        ...
        /// </summary>
        /// <returns>Db</returns>
        public static Database FromConfig() => FromConfig(ConfigurationManager.ConnectionStrings[0]);

        /// <summary>
        /// Factory method, instantiating the Db class from a named connectionstring 
        /// in the app.config or web.config file.
        /// </summary>
        public static Database FromConfig(string connectionStringName)
            => FromConfig(ConfigurationManager.ConnectionStrings[connectionStringName]);

        private static Database FromConfig(ConnectionStringSettings connectionStringSettings)
        {
        ...
        internal Database(string connectionString, DbConfig config, DbProviderFactory connectionFactory)
        {
